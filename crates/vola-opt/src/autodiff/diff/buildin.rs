...
use crate::{
    ...
    autodiff::{activity::Activity, ad_forward::ForwardAd, AdResponse},
    ...
    pub(super) fn build_diff_buildin(
        ...
        match op {
            ...
            BuildinOp::SquareRoot => {
                ...
                let ty = self.opt.get_out_type_mut(f_src).unwrap();
                let imm_one = self
                    .opt
                    .splat_scalar(region, ImmScalar::new(1.0), ty.clone());
                let imm_two = self.opt.splat_scalar(region, ImmScalar::new(2.0), ty);
            ...
            BuildinOp::Min | BuildinOp::Max => {
                ...
                let right_src = self.opt.graph.inport_src(node.input(0)).unwrap();
                ...
                let zero_splat = self.opt.emit_zero_for_port(region, node.output(0));
            ...
            BuildinOp::Exp => {
                ...
                Ok(self.build_chain_rule_for(
                    ...
                    node.output(0),
                    node.output(0),
                    self.opt.graph.inport_src(node.input(0)).unwrap(),
                ))
            }
            BuildinOp::Pow => {
                ...
                let n_src = self.opt.graph.inport_src(node.input(1)).unwrap();

                if activity.is_active_port(self.opt, n_src) {
                    //the x^x case
                    ...
                    //the x^n case
                    let nty = self.opt.get_out_type_mut(n_src).unwrap();
                    let one = self.opt.splat_scalar(region, ImmScalar::new(1.0), nty);
