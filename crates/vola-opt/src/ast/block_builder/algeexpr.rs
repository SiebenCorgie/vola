...
impl<'a> BlockBuilder<'a> {
    ...
    pub fn setup_alge_expr(&mut self, expr: Expr) -> Result<OutportLocation, OptError> {
        ...
        match expr.expr_ty {
            ...
            ExprTy::Tuple(tuple_elements) => {
                compile_error!("Handle Tuple creation via extra constructor?");
            }
            ...
            ExprTy::BranchExpr(e) => self.setup_gamma_expr(*e, expr_span),
        }
    }
    ...
    fn setup_gamma_expr(
        ...
        //
        //2. Setup the new variable_defs at the end of this gamma. This works
        //   by recording all-defs we write to, and overwriting them once we are done.

        compile_error!("Setup new gamma-builder")
        /*
            ...
            assert!(gamma.conditional.len() >= 1);
    ...
    fn setup_theta_expr(
        ...
        //head expression. This is needed, since the RVSDG defines theta nodes as tail controlled.
        ...
        compile_error!("Implement-new-loop")
        /*
            struct PortPackage {
    ...
    fn setup_call_expr(
        ...
        if let Some(wknode) = OptNode::try_parse(&call.ident.0) {
            ...
            if let Some(algefn) = self.opt.functions.get(&call.ident.0).cloned() {
                let alge_import =
