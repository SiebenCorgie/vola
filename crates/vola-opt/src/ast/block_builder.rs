...
impl BlockBuilderConfig {
    pub fn impl_block(csgnode_ty: CsgTy) -> Self {
        BlockBuilderConfig {
            allow_eval: csgnode_ty == CsgTy::Operation,
            expect_return: RetType::Value,
...
impl<'a> BlockBuilder<'a> {
    pub fn inherite_ctx<'b>(
        &'b mut self,
        span: Span,
        config: BlockBuilderConfig,
        return_type: SmallColl<Ty>,
    ) -> BlockBuilder<'b> {
        BlockBuilder {
            span,
            config,
        ...
        if !self.config.allow_eval {
            ...
            report(
                error_reporter(err.clone(), eval_expr.span.clone())
                    .with_label(
                        Label::new(eval_expr.span.clone()).with_message(
                            "consider moving this eval into a concept implementation.",
                        ),
                    )
                    .finish(),
            );
        ...
        match &self.opt.graph.node(self.parent_node()).node_type {
            NodeType::Gamma(_) => {
                ...
                let lmd = self
                    .opt
                    .graph
                    .find_parent_lambda_or_phi(self.parent_node())
                    .expect("expected gamma node to exist withing lambda region");
                ...
                let gamma_port = self
                    .opt
                    .import_argument(src_port, self.region)
            ...
            NodeType::Theta(_) => {
                ...
                let lmd = self
                    ...
                    .graph
                    .find_parent_lambda_or_phi(self.parent_node())
                    .expect("expected theta node to exist withing lambda region");
                let src_port = OutportLocation {
                    node: lmd,
                    output: OutputType::ContextVariableArgument(cv),
                };
            ...
            NodeType::Lambda(_) => OutportLocation {
    ...
    pub(crate) fn build_block(
        ...
        self.opt
            .span_tags
            .set(self.region.into(), self.span.clone());
            ...
            match stmt {
                Stmt::Csg(csgbind) => {
                    if !self.config.allow_csg_stmt {
                        let err = OptError::Any {
                            text: format!("Csg Statement not allowed in this context"),
                        };
                        report(
                            error_reporter(err.clone(), csgbind.span.clone())
                                .with_label(
                                    Label::new(csgbind.span.clone()).with_message(
                                        "csg binding is not allowed in this context!",
                                    ),
                                )
                                .finish(),
                        );
                ...
                Stmt::Branch(_branch) => todo!("setup branch stmt"),
                Stmt::Loop(_loop) => todo!("setup loop stmt"),
            }
        }

        let return_expression = if let Some(expr) = block.retexpr {
            match self.config.expect_return {
                RetType::Value => Some(expr),
                RetType::None => {
                    let err = OptError::Any {
                        text: format!("Expected no return value"),
                    };
                    report(
                        error_reporter(err.clone(), block.span.clone())
                            .with_label(Label::new(block.span.clone()).with_message(
                                "consider removing the last expression from the end of this block",
                            ))
                            .finish(),
                    );
                    return Err(err);
        ...
        match (self.config.expect_return.clone(), return_expression) {
            (RetType::Value, Some(e)) => {
                let expr = self.setup_expr(e)?;
            ...
            (RetType::Value, None) => {
                let err = OptError::Any {
                    text: format!("Expected a algebraic return statment at the end"),
                };
                report(
                    error_reporter(err.clone(), block.span.clone())
                        .with_label(
            ...
            (RetType::CsgTree, Some(ReturnExpr::CsgOp(op))) => {
                Ok(smallvec![(Some(Ty::CSGTree), self.setup_csg_tree(op)?)])
            }
            (RetType::CsgTree, None) => {
                let err = OptError::Any {
                    text: format!("Expected a csg return statment at the end"),
                };
                report(
                    error_reporter(err.clone(), block.span.clone())
                        .with_label(Label::new(block.span.clone()).with_message(
                            "consider adding a csg-expression at the end of this block",
                        ))
                        .finish(),
                );
                Err(err)
            ...
            (RetType::AccessDesc, Some(ReturnExpr::AccessDescriptors(ad))) => {
                let access = self
                    .build_access(ad)?
                    .into_iter()
                    .map(|(ty, port)| (Some(ty), port))
                    .collect();
                Ok(access)
            }
            (RetType::AccessDesc, None) => {
                let err = OptError::Any {
                    text: format!("Expected a access-description at the end of this block"),
                };
                report(
                    error_reporter(err.clone(), block.span.clone())
                        .with_label(Label::new(block.span.clone()).with_message(
                            "consider adding a one, or a tuple of multiple eval expressions",
                        ))
                        .finish(),
                );
                Err(err)
            ...
            (RetType::None, Some(any)) => {
                ...
                let err = OptError::Any {
                    text: format!("Expected no return statment, but had a {}", any.describe()),
                };
                report(
                ...
                match self.opt.import_argument(var.port, self.region) {
                    Ok(new_known_port) => {
                        ...
                        assert!(self
                            .lmd_ctx
                            .defined_vars
                            .insert(var_name.to_owned(), new_var_def.clone())
                            .is_some());
                        //and overwrite producer tag
                        self.opt
                            .var_producer
                            .set(new_var_def.port.into(), var_name.to_owned());
                        Some(new_var_def)
                    }
                    Err(e) => {
